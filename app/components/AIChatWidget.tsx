          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700"
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-teal-400">Bill assistant</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 scrollbar-thin scrollbar-thumb-slate-700">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'assistant' && (
                  <div className="w-6 h-6 rounded-full bg-teal-800 flex items-center justify-center mr-2 flex-shrink-0 mt-1">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {QUICK_PROMPTS.map(p => (
                    <button
                      key={p}
                      onClick={() => send(p)}
                      className="text-xs px-2 py-2 rounded-xl border border-teal-800 text-teal-400 bg-teal-950/40 hover:bg-teal-900/40 hover:border-teal-600 transition-colors text-left leading-tight"
                    >
                      {p}
                    </button>
                  ))}
          ...
          <div className="px-3 py-3 border-t border-slate-700" style={{ background: '#0f172a' }}>
            ...
            >
              <input
                ...
                disabled={loading}
                className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-teal-600 disabled:opacity-50 transition-colors"
              />
              <button
                type="submit"
