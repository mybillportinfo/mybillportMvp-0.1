...
export function LogoIcon({ size = 32, className = '' }: { size?: number; className?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      ...
      <circle cx="36" cy="36" r="3" fill="#34d399" />
      <defs>
    ...
    <div className={`flex items-center gap-2.5 ${className}`}>
      ...
      <div className="flex flex-col">
        <span className="font-bold tracking-tight text-white leading-none" style={{ fontSize: textHeight }}>
          My<span className="text-teal-400">Bill</span>Port
        </span>
      </div>
    </div>
  );
...
export function LogoIconAlt({ size = 48, className = '' }: { size?: number; className?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      ...
      <path d="M30 26l2.5 2.5L37 24" stroke="#34d399" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      <defs>
