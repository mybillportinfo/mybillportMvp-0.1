    ...
    <div className="min-h-screen bg-[#0b1426] text-white">
      <div className="max-w-4xl mx-auto px-5 py-16 space-y-16">
        <section className="text-center space-y-6">
          <div className="inline-flex items-center gap-2 bg-[#2AC3B5]/10 border border-[#2AC3B5]/20 text-[#2AC3B5] text-xs font-semibold px-4 py-1.5 rounded-full tracking-wide uppercase">
        ...
        <section className="space-y-6">
          ...
          <div className="bg-[#111d33] border border-white/5 rounded-xl p-6 space-y-4">
            <div className="flex gap-4 items-start">
              <span className="text-[#FF8A5C] font-bold text-lg">1.</span>
              <p className="text-slate-300">Add your bill to MyBillPort — snap a photo, upload a PDF, or type it in manually.</p>
            </div>
            <div className="flex gap-4 items-start">
              <span className="text-[#FF8A5C] font-bold text-lg">2.</span>
            ...
            <div className="flex gap-4 items-start">
              <span className="text-[#FF8A5C] font-bold text-lg">3.</span>
        ...
        <section className="space-y-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {categories.map((c) => (
              <div key={c.name} className="bg-[#111d33] border border-white/5 rounded-xl p-5 space-y-3">
                <h3 className="font-semibold text-white">{c.name}</h3>
                <ul className="space-y-1.5">
                  {c.examples.map((b) => (
                    <li key={b} className="text-sm text-slate-400 flex items-center gap-2">
                      <ExternalLink className="w-3.5 h-3.5 text-[#2AC3B5]" />
                      {b}
                    </li>
                  ))}
                </ul>
