...
export const parseRogers: BillerParser = (text: string): ParsedBill | null => {
  const identifiers = /rogers\s*(communications|wireless|internet|ignite)?/i;
  if (!identifiers.test(text)) return null;

  const accountRegex = /Account\s*(?:Number|#|No)?\s*:?\s*(\d{3}[-\s]?\d{3}[-\s]?\d{4})/i;
