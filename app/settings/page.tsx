  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: '#F8FAFC' }}>
      ...
      <div className="relative overflow-hidden px-5 pt-12 pb-7">
        ...
        <Link href="/app" className="flex items-center text-[#64748B] hover:text-[#0F172A] mb-5 transition-colors relative">
          <ArrowLeft className="w-5 h-5 mr-2" />
          Back to Dashboard
        </Link>
        <div className="flex items-center gap-3 relative">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center shadow-lg" style={{ background: 'linear-gradient(135deg, #5B5BE6, #4646CC)' }}>
            <Settings className="text-white w-6 h-6" />
          </div>
          <div>
            <span className="text-[#0F172A] font-bold text-xl tracking-tight">Settings</span>
      ...
      <div className="px-4 space-y-3">
        ...
        >
          <div className="p-4 flex items-center gap-4">
            {photoURL ? (
              <img src={photoURL} alt="Profile" className="w-14 h-14 rounded-full object-cover" style={{ border: '2.5px solid #5B5BE6' }} />
            ) : (
              <div className="w-14 h-14 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #5B5BE6, #4DA85A)' }}>
                <User className="w-7 h-7 text-white" />
              </div>
            )}
            <div className="flex-1 text-left">
              <p className="font-semibold text-[#0F172A]">{username || user.displayName || 'MyBillPort User'}</p>
              <p className="text-sm text-slate-400">{user.email}</p>
            </div>
            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'rgba(107,203,119,0.15)' }}>
              <ChevronRight className="w-4 h-4 text-[#5B5BE6]" />
            </div>
          </div>
        </button>
        ...
        <div className="rounded-2xl p-4 space-y-3" style={{ background: 'linear-gradient(135deg, rgba(255,179,71,0.1), rgba(77,106,159,0.08))', border: '1px solid rgba(255,179,71,0.25)' }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-[#0F172A]">{isPremium ? 'Premium Plan' : 'Free Plan'}</p>
        ...
        <div className="rounded-2xl p-4 space-y-3" style={{ background: 'rgba(77,106,159,0.12)', border: '1px solid rgba(77,106,159,0.3)' }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #5B5BE6, #4646CC)' }}>
              <Forward className="w-5 h-5 text-white" />
            </div>
            <div className="flex-1">
              <p className="font-semibold text-[#0F172A]">Bill Forwarding Email</p>
              <p className="text-xs text-slate-400">Forward bills for auto-detection</p>
            </div>
            <Link href="/pending-bills" className="text-xs text-[#5B5BE6] font-semibold hover:underline flex items-center gap-1">
          ...
          {emailAlias ? (
            ...
            <div className="h-8 flex items-center"><Loader2 className="w-4 h-4 text-[#5B5BE6] animate-spin" /></div>
          )}
        </div>
        ...
        <div className="rounded-2xl overflow-hidden divide-y divide-[#E2E8F0]" style={{ background: '#FFFFFF', border: '1px solid #E2E8F0' }}>
          <button onClick={() => setActiveModal('referral')} className="w-full p-4 flex items-center gap-4 hover:bg-slate-50 transition-colors">
            ...
            <div className="flex-1 text-left">
              <span className="text-[#0F172A] font-medium">Refer a Friend</span>
          ...
          <button onClick={() => setActiveModal('security')} className="w-full p-4 flex items-center gap-4 hover:bg-slate-50 transition-colors">
            ...
            <span className="flex-1 text-left text-[#0F172A] font-medium">Security</span>
            <ChevronRight className="w-4 h-4 text-slate-500" />
          </button>
        </div>
            ...
            {activeModal === 'profile' && (
              <div>
                <div className="flex items-center justify-between p-5 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#5B5BE6]/15 rounded-lg flex items-center justify-center">
                      <User className="w-5 h-5 text-[#5B5BE6]" />
                    </div>
                ...
                <div className="p-5 space-y-5">
                  <div className="flex flex-col items-center gap-3">
                    <div className="relative">
                      {photoURL ? (
                        <img src={photoURL} alt="Profile" className="w-20 h-20 rounded-full object-cover border-2 border-[#5B5BE6]/30" />
                      ) : (
                        <div className="w-20 h-20 bg-[#5B5BE6]/15 rounded-full flex items-center justify-center">
                          <User className="w-10 h-10 text-[#5B5BE6]" />
                        </div>
                      )}
                      <button
                        onClick={() => photoInputRef.current?.click()}
                        className="absolute -bottom-1 -right-1 w-8 h-8 bg-[#5B5BE6] rounded-full flex items-center justify-center shadow-lg hover:bg-[#5B5BE6] transition-colors"
                      >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#5B5BE6] focus:border-transparent"
                    />
                  </div>
                  ...
                  {profileSaved && (
                    <div className="bg-[#5B5BE6]/10 border border-[#5B5BE6]/30 text-[#4646CC] px-4 py-2 rounded-lg text-sm text-center flex items-center justify-center gap-2">
            ...
            {activeModal === 'referral' && (
              <div>
                <div className="flex items-center justify-between p-5 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#5B5BE6]/15 rounded-lg flex items-center justify-center">
                      <Gift className="w-5 h-5 text-[#5B5BE6]" />
                    </div>
                    <div>
                ...
                <div className="p-5 space-y-5">
                  <div className="bg-gradient-to-br from-[#5B5BE6]/10 to-slate-50 border border-[#5B5BE6]/30 rounded-xl p-4 space-y-3">
                    <p className="text-sm font-medium text-slate-700">Your referral code</p>
                    {referralCode ? (
                      <>
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-white border-2 border-[#5B5BE6]/40 rounded-lg px-4 py-3 text-center">
                            <span className="text-2xl font-bold text-[#4646CC] tracking-[0.3em]">{referralCode}</span>
                          </div>
                          <button
                            onClick={() => {
                            ...
                            className="p-3 bg-[#5B5BE6] text-white rounded-lg hover:bg-[#5B5BE6] transition-colors"
                          >
                        ...
                        <button
                          ...
                          className="w-full text-sm text-[#5B5BE6] hover:text-[#4646CC] flex items-center justify-center gap-1.5 py-1"
                        >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-slate-50 rounded-xl p-4 text-center">
                      <Users className="w-6 h-6 text-[#5B5BE6] mx-auto mb-1" />
                    ...
                    <div className="bg-slate-50 rounded-xl p-4 text-center">
                      <Gift className="w-6 h-6 text-[#5B5BE6] mx-auto mb-1" />
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 space-y-3">
                    ...
                    <div className="space-y-2.5">
                      <div className="flex items-start gap-3">
                        <span className="w-6 h-6 rounded-full bg-[#5B5BE6] text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">1</span>
                        <p className="text-sm text-slate-600">Share your referral code or invite link with a friend</p>
                      </div>
                      <div className="flex items-start gap-3">
                        <span className="w-6 h-6 rounded-full bg-[#5B5BE6] text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">2</span>
                        <p className="text-sm text-slate-600">They enter your code when signing up for MyBillPort</p>
                      </div>
                      <div className="flex items-start gap-3">
                        <span className="w-6 h-6 rounded-full bg-[#5B5BE6] text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">3</span>
                  ...
                  {(ctxProfile?.referralFreeMonths ?? 0) > 0 && (
                    <div className="bg-[#5B5BE6]/10 border border-[#5B5BE6]/30 rounded-xl p-4 flex items-center gap-3">
                      <Gift className="w-5 h-5 text-[#5B5BE6] flex-shrink-0" />
                      <p className="text-sm text-[#4646CC]">
            ...
            {activeModal === 'notifications' && (
              <div>
                <div className="flex items-center justify-between p-5 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#5B5BE6]/15 rounded-lg flex items-center justify-center">
                      <Bell className="w-5 h-5 text-[#5B5BE6]" />
                    </div>
                    ...
                    <>
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`w-12 h-7 rounded-full transition-colors relative ${inAppReminders ? 'bg-[#5B5BE6]' : 'bg-slate-300'}`}
                        >
                            ...
                            <label key={day} className="flex items-center gap-3 cursor-pointer">
                              <div
                                ...
                                className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors cursor-pointer ${
                                  notifyDays.includes(day) ? 'bg-[#5B5BE6] border-[#5B5BE6]' : 'border-slate-300 bg-white'
                                }`}
                              >
                      ...
                      <div className="border-t border-slate-100 pt-4">
                        <div className="flex items-center justify-between">
                          ...
                          {pushSupported && pushPermission !== 'denied' && (
                            <button
                              ...
                              className={`w-12 h-7 rounded-full transition-colors relative disabled:opacity-50 ${pushSubscribed ? 'bg-[#5B5BE6]' : 'bg-slate-300'}`}
                            >
                        ...
                        {pushSubscribed && (
                          <p className="text-xs text-[#5B5BE6] mt-2 ml-8">
                      ...
                      {prefsSaved && (
                        <div className="bg-[#5B5BE6]/10 border border-[#5B5BE6]/30 text-[#4646CC] px-4 py-2 rounded-lg text-sm text-center">
                          Preferences saved!
                        </div>
                      )}
                ...
                <div className="p-5 space-y-5">
                  ...
                  <div className="bg-slate-50 rounded-lg p-4 space-y-3">
                    ...
                    <ul className="text-sm text-slate-600 space-y-2">
                      <li className="flex items-start gap-2"><span className="text-[#5B5BE6] mt-0.5">&#10003;</span>All your data is encrypted and stored securely</li>
                      <li className="flex items-start gap-2"><span className="text-[#5B5BE6] mt-0.5">&#10003;</span>We never sell or share your personal information</li>
                      <li className="flex items-start gap-2"><span className="text-[#5B5BE6] mt-0.5">&#10003;</span>You can request data deletion at any time</li>
                    </ul>
                  </div>
              ...
              <div>
                ...
                <div className="p-5 space-y-5">
                  ...
                  {biometricSupported && (
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative w-12 h-7 rounded-full transition-colors ${biometricEnabled ? 'bg-[#5B5BE6]' : 'bg-slate-300'} ${biometricLoading ? 'opacity-50' : ''}`}
                      >
                        {biometricLoading ? (
                  ...
                  <div className="bg-slate-50 rounded-lg p-4 space-y-3">
                    ...
                    <ul className="text-sm text-slate-600 space-y-2">
                      <li className="flex items-start gap-2"><span className="text-[#5B5BE6] mt-0.5">&#10003;</span>Your password is securely hashed</li>
                      <li className="flex items-start gap-2"><span className="text-[#5B5BE6] mt-0.5">&#10003;</span>Sessions expire automatically for safety</li>
                      <li className="flex items-start gap-2"><span className="text-[#5B5BE6] mt-0.5">&#10003;</span>All connections use HTTPS encryption</li>
                    </ul>
                  </div>
