    ...
    for (const msg of allMessages.slice(0, MAX_EMAILS_TO_PROCESS)) {
      try {
        ...
        const pendingBill: Omit<PendingBill, 'id'> = {
          ...
          billerDomain: billerDomain ?? null,
          amount: storedAmount,
          dueDate: storedDueDate,
          accountNumber: storedAccountNum,
          accountNumberDisplay: finalAccountNumDisplay ?? null,
          statementDate: finalStmtDate ?? null,
          minimumPayment: finalMinPayment ?? null,
          totalBalance: finalTotalBal ?? null,
          ...
          status: 'pending',
          createdAt: Date.now(),
          // null instead of undefined so Firestore doesn't reject the document
          matchedProviderId: provider?.providerId ?? null,
          matchedProviderName: provider?.providerName ?? null,
