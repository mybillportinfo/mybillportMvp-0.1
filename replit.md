...
- **Biometric Payment Authentication**: WebAuthn/FIDO2 biometric verification (fingerprint/face) before bill payments. Toggle in Settings > Security. Uses `@simplewebauthn/server` + `@simplewebauthn/browser`. Credentials stored in `userProfiles/{uid}` (biometricEnabled, biometricCredentialId, biometricPublicKey, biometricCounter). Challenges stored in `webauthnChallenges/{uid}`. API routes: /api/webauthn/register, /api/webauthn/verify-registration, /api/webauthn/authenticate, /api/webauthn/verify-authentication, /api/webauthn/disable.
- **Branding & SEO**: Professional landing page with Logo component (SVG, 2 concepts + monochrome + favicon variants), trust badges, testimonials, social proof stats, FAQ accordion with ARIA. SEO: JSON-LD structured data (Organization, WebSite, SoftwareApplication, FAQPage), Open Graph/Twitter meta tags, sitemap.xml (via app/sitemap.ts), robots.txt. Programmatic SEO pages: /bill-management-canada, /bill-tracker, /pay-bills-online-canada.
...
- `webauthnChallenges`: Temporary WebAuthn challenges (keyed by userId, auto-expire after 5 min).
...
- **@simplewebauthn/server** + **@simplewebauthn/browser**: WebAuthn/FIDO2 biometric authentication for payment verification.